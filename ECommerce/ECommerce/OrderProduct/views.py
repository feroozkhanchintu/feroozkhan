...
class OrderLineItemViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if orderLineItemSerializer.is_valid():
            product = get_object_or_404(models.Product, id=data['product_id'], isavailable=True)
            order = get_object_or_404(models.Orders, order_id=kwargs['order_id'], deleted = False)
...
def ProductSummary(request):
    ...
    if groupby :
        if query.count() != 0:
            ...
            query = {'count' : 0}
            if(categoryname) :
                query['category_id'] = models.Category.objects.get(name=categoryname).id
            return JsonResponse([query], safe=False)
    ...
    if groupby:
        if query.count() !=0 :
            ...
            if 'product' in groupby:
                ...
                query['product_id'] = models.Product.objects.get(product_id=productcode).id

            return JsonResponse([query], safe=False)

    else:
        query = query.values('order__order_id').distinct().aggregate(count=Count('order__order_id'))
        return JsonResponse([query], safe=False)
